...
func init() {
	...
	orm.RegisterModel(
		new(Address), new(Asset), new(GasNow),
		new(TokenConfig),new(TxRecord), new(MarketAsset),
		new(AddrNoteBook), new(Market), new(News),
		new(Chain), new(Version), new(AdminUser),
		new(AdminMenu), new(AdminRole), new(WalletStat),
	)
